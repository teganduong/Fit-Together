...
exports.addActivity = (req, res) => {
  db.one('insert into daily_activities(type, category, description, date_performed, count, units)' + 
      'values(${type}, ${category}, ${description}, ${date_performed}, ${count}, ${units}) returning id', req.body)
    .then((activityId) => {
      const userActivity = {
      ...
      return db.none('insert into users_daily_activities(user_id, daily_activity_id)' + 
        'values(${user_id}, (select id from daily_activities where id=${daily_activity_id}))', userActivity);
    })
    .then(() => {
      res.status(201)
        .json({
